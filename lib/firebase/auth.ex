defmodule Firebase.Auth do
  ...
  ## Examples
    ...
    iex> Firebase.Auth.verify_token("eyJhbGciOiJS...")
    {:ok,
     %JOSE.JWT{
       fields: %{
  ...
  def verify_token(token) do
    with %JOSE.JWT{
         ...
         {:ok, key} <- Firebase.KeyManager.get_key(key_id) do
  ...
  defp verify_jwt_signature_with_key(signed_jwt, public_key) do
    with %JOSE.JWK{} = jwk <- JOSE.JWK.from_pem(public_key),
         {true, %JOSE.JWT{} = jwt, %JOSE.JWS{}} <-
           JOSE.JWT.verify_strict(jwk, ["RS256"], signed_jwt) do
      {:ok, jwt}
    end
  end

  defp project_id do
    Application.get_env(:firebase, :project_id)
  end
end
